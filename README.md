...
高并发下,大量请求的数据在缓存和数据库都不存在,导致大量请求打到数据库
...
![image-20260508095417607](C:\Users\Charon\AppData\Roaming\Typora\typora-user-images\image-20260508095417607.png)

**什么是缓存雪崩，怎么解决？**
![image](https://github.com/user-attachments/assets/758bf96d-af23-4d89-91de-71f96c001be6)


 *定义：*同一时间大量key同时失效或者redis服务宕机,导致大量请求打到数据库

造成缓存雪崩问题的原因是是大量key拥有了相同的过期时间，比如对课程信息设置缓存过期时间为10分钟，在大量请求同时查询大量的课程信息时，此时就会有大量的课程存在相同的过期时间，一旦失效将同时失效，造成雪崩问题。

*解决方法：*

![image-20260508100835038](C:\Users\Charon\AppData\Roaming\Typora\typora-user-images\image-20260508100835038.png)

```java

降级限流策略
信号量限制（Semaphore）：
控制整个后端查询数据库的总并发数。比如设置最多只有 100 个线程能同时查库，剩下的请求直接拒绝或返回“系统繁忙”。

Sentinel 熔断降级：

这是图片中提到的“给业务添加降级限流策略”。当发现数据库压力过大或 Redis 挂了，直接走降级逻辑（返回默认数据或错误提示），不再尝试查库。
```
...
*定义：*大量并发访问同一个热点数据,并且缓存的数据失效,导致大量请求打到数据库
...
开启子线程重建缓存的作用在于提高系统的响应速度，避免因缓存击穿导致的数据库压力过大，同时保障系统在高并发场景下的稳定性，
...
![image-20260509113634936](C:\Users\Charon\AppData\Roaming\Typora\typora-user-images\image-20260509113634936.png)
